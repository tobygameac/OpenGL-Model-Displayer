...
public:
  ...
  void Draw(const GLShader &gl_shader, const glm::mat4 &parent_modelview_matrix) {
    ...
    mesh_->Draw(gl_shader, parent_modelview_matrix);

    if (next_mesh_node_ != nullptr) {
      next_mesh_node_->Draw(gl_shader, parent_modelview_matrix);
    }

    if (child_mesh_node_ != nullptr) {
      glm::mat4 modelview_matrix = parent_modelview_matrix * mesh_->GetModelviewMatrixWithAnimation();
      child_mesh_node_->Draw(gl_shader, modelview_matrix);
    }
  }
  ...
  void SetMeshTextureId(const GLuint texture_id) {
    ...
    if (next_mesh_node_ != nullptr) {
      next_mesh_node_->SetMeshTextureId(texture_id);
    }

    if (child_mesh_node_ != nullptr) {
      child_mesh_node_->SetMeshTextureId(texture_id);
    }
  }
...
protected:
  ...
  virtual void Draw(const GLShader &gl_shader) {
    root_mesh_node_->Draw(gl_shader, glm::mat4(1.0));
  }

  virtual void Draw(const GLShader &gl_shader, const glm::mat4 &root_modelview_matrix) {
    root_mesh_node_->Draw(gl_shader, root_modelview_matrix);
  }
